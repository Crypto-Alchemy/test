...
class ZendeskSessionStorage implements SessionStorage {
    ...
    private static void clearDirectory(File file) {
        if (file != null && file.exists()) {
            ...
            if (listFiles != null && file.isDirectory() && kj0.m47119j(listFiles)) {
    ...
    private File createDirIfNotExists(File file) {
        if (file.exists()) {
            Logger.m43078b(LOG_TAG, "Created dir %s, success: %s", file.getAbsolutePath(), Boolean.valueOf(file.mkdirs()));
        }
        return file;
    }
