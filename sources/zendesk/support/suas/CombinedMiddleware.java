...
class CombinedMiddleware implements Middleware {
    ...
    public void loopThroughMiddleware(Action<?> action, GetState getState, Dispatcher dispatcher, Continuation continuation, Iterator<Middleware> it) {
        if (it.hasNext()) {
            final GetState getState2 = getState;
            final Dispatcher dispatcher2 = dispatcher;
            final Continuation continuation2 = continuation;
            final Iterator<Middleware> it2 = it;
            it.next().onAction(action, getState, dispatcher, new Continuation() {
                public void next(Action<?> action) {
                    CombinedMiddleware.this.loopThroughMiddleware(action, getState2, dispatcher2, continuation2, it2);
                }
            });
            return;
        }
