    ...
    public StateAttachments reduce(StateAttachments stateAttachments, Action<?> action) {
        ...
        switch (c) {
            case 0:
                ...
                return stateAttachments.newBuilder().addAllSelectedAttachments(arrayList).setSelectedAttachments(kj0.m47111b(arrayList, stateAttachments.getSelectedAttachments())).build();
            case 1:
                HashSet hashSet = new HashSet();
                for (MediaResult i : (List) action.getData()) {
            ...
            case 2:
                ...
                if (stateSettings.isAttachmentsEnabled()) {
                    ...
                    for (StateRequestAttachment next2 : stateAttachments.getSelectedAttachments()) {
                        if (next2.getSize() > maxAttachmentSize) {
                            Logger.m43078b(RequestActivity.LOG_TAG, LOG_FORMAT_ATTACHMENT_DROPPED, next2.getName(), Long.valueOf(next2.getSize()), Long.valueOf(maxAttachmentSize));
                        } else {
                ...
                Logger.m43087k(RequestActivity.LOG_TAG, LOG_MESSAGE_ATTACHMENTS_DISABLED, new Object[0]);
                return new StateAttachments();
            case 3:
