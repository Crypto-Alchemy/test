...
public enum Support {
    ...
    public void init(Zendesk zendesk2) {
        ...
        Logger.m43081e(LOG_TAG, "Cannot use SupportSDK without initializing Zendesk. Call Zendesk.INSTANCE.init(...)", new Object[0]);
    }
    ...
    public boolean isAuthenticated() {
        ...
        if (authenticationProvider2 == null || authenticationProvider2.getIdentity() == null) {
            return false;
        }
        return true;
    }
    ...
    public boolean refreshRequest(String str, Context context) {
        if (!isInitialized()) {
            Logger.m43081e(LOG_TAG, "Cannot use Support SDK without initializing Zendesk. Call Zendesk.INSTANCE.init(...) and Support.INSTANCE.init(Zendesk)", new Object[0]);
            return false;
        } else if (!isAuthenticated()) {
            Logger.m43081e(LOG_TAG, "Cannot use Support SDK without setting an identity. Zendesk.INSTANCE.setIdentity(...) must be called before refreshRequest(...).", new Object[0]);
            return false;
        } else {
