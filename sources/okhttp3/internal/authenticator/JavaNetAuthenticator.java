...
public final class JavaNetAuthenticator implements Authenticator {
    ...
    private final InetAddress connectToInetAddress(Proxy proxy, HttpUrl httpUrl, Dns dns) throws IOException {
        ...
        if (i == 1) {
            return (InetAddress) CollectionsKt___CollectionsKt.m47329Z(dns.lookup(httpUrl.host()));
        }
        SocketAddress address = proxy.address();
        if (address != null) {
            InetAddress address2 = ((InetSocketAddress) address).getAddress();
            vx2.m53590f(address2, "address() as InetSocketAddress).address");
            return address2;
        }
    ...
    public Request authenticate(Route route, Response response) throws IOException {
        ...
        for (Challenge next : challenges) {
            if (yb6.m74344y("Basic", next.scheme(), true)) {
                ...
                if (passwordAuthentication != null) {
                    if (z) {
                        ...
                        str = Constants.AUTHORIZATION_HEADER;
                    }
                    String userName = passwordAuthentication.getUserName();
                    vx2.m53590f(userName, "auth.userName");
                    char[] password = passwordAuthentication.getPassword();
                    vx2.m53590f(password, "auth.password");
