package net.safemoon.androidwallet.fragments;

import android.text.Editable;
import com.google.android.material.chip.Chip;
import kotlin.Metadata;
import kotlin.jvm.internal.Lambda;
import kotlin.text.StringsKt__StringsKt;

@Metadata(mo44875bv = {}, mo44876d1 = {"\u0000\u0010\n\u0002\u0010\b\n\u0002\b\u0002\n\u0002\u0018\u0002\n\u0002\b\u0003\u0010\u0006\u001a\u00020\u00032\u000e\u0010\u0002\u001a\n \u0001*\u0004\u0018\u00010\u00000\u0000H\n¢\u0006\u0004\b\u0004\u0010\u0005"}, mo44877d2 = {"", "kotlin.jvm.PlatformType", "it", "Lr37;", "invoke", "(Ljava/lang/Integer;)V", "<anonymous>"}, mo44878k = 3, mo44879mv = {1, 7, 1})
/* compiled from: SwapFragment.kt */
public final class SwapFragment$onViewCreated$39 extends Lambda implements rc2<Integer, r37> {
    public final /* synthetic */ SwapFragment this$0;

    /* JADX INFO: super call moved to the top of the method (can break code semantics) */
    public SwapFragment$onViewCreated$39(SwapFragment swapFragment) {
        super(1);
        this.this$0 = swapFragment;
    }

    public /* bridge */ /* synthetic */ Object invoke(Object obj) {
        invoke((Integer) obj);
        return r37.f33317a;
    }

    public final void invoke(Integer num) {
        String obj;
        xa2 u0 = this.this$0.f42150X;
        xa2 xa2 = null;
        if (u0 == null) {
            vx2.m53605u("binding");
            u0 = null;
        }
        boolean z = true;
        u0.f35564b.setVisibility(ol0.m70370l0(num != null && num.intValue() == 1));
        xa2 u02 = this.this$0.f42150X;
        if (u02 == null) {
            vx2.m53605u("binding");
            u02 = null;
        }
        Chip chip = u02.f35586x;
        vx2.m53590f(chip, "binding.requireGasFee");
        chip.setVisibility(8);
        if (num != null && num.intValue() == 1) {
            xa2 u03 = this.this$0.f42150X;
            if (u03 == null) {
                vx2.m53605u("binding");
                u03 = null;
            }
            u03.f35565c.setVisibility(8);
            xa2 u04 = this.this$0.f42150X;
            if (u04 == null) {
                vx2.m53605u("binding");
            } else {
                xa2 = u04;
            }
            xa2.f35566d.setVisibility(8);
        } else {
            fe7 w0 = this.this$0.f42160q1;
            if (w0 == null) {
                vx2.m53605u("bindingSource");
                w0 = null;
            }
            Editable text = w0.f28840m.getText();
            String obj2 = (text == null || (obj = text.toString()) == null) ? null : StringsKt__StringsKt.m63090V0(obj).toString();
            if (!(obj2 == null || obj2.length() == 0)) {
                z = false;
            }
            if (z) {
                xa2 u05 = this.this$0.f42150X;
                if (u05 == null) {
                    vx2.m53605u("binding");
                    u05 = null;
                }
                u05.f35565c.setVisibility(0);
                xa2 u06 = this.this$0.f42150X;
                if (u06 == null) {
                    vx2.m53605u("binding");
                    u06 = null;
                }
                u06.f35566d.setVisibility(8);
                xa2 u07 = this.this$0.f42150X;
                if (u07 == null) {
                    vx2.m53605u("binding");
                } else {
                    xa2 = u07;
                }
                xa2.f35564b.setVisibility(8);
            } else {
                xa2 u08 = this.this$0.f42150X;
                if (u08 == null) {
                    vx2.m53605u("binding");
                    u08 = null;
                }
                u08.f35565c.setVisibility(8);
                xa2 u09 = this.this$0.f42150X;
                if (u09 == null) {
                    vx2.m53605u("binding");
                    u09 = null;
                }
                u09.f35566d.setVisibility(0);
                xa2 u010 = this.this$0.f42150X;
                if (u010 == null) {
                    vx2.m53605u("binding");
                } else {
                    xa2 = u010;
                }
                xa2.f35564b.setVisibility(8);
            }
        }
        if (num != null && num.intValue() == 0) {
            a77.m55425c();
        }
    }
}
